...
GLSLProgram*	Pattern;				// pattern for shaders
...
Animate()
{
	...
	// for Display( ) to find:
	const int MS_IN_THE_ANIMATION_CYCLE = 20000;	// milliseconds in the animation loop
...
Display()
{
	...
	if (VertShader == true)
	{
		TimeVert = 0.5 + 0.5 * sin(2. * M_PI * Time);
	}
	Pattern->SetUniformVariable("uTimeV", TimeVert);

	if (FragShader == true)
	{
		TimeFrag = 0.5 + 0.5 * sin(2. * M_PI * Time);
	}
	Pattern->SetUniformVariable("uTimeF", TimeFrag);

	GLfloat color[] = { 1., 0., 0. };
	Pattern->SetUniformVariable("uColor", color);
	Pattern->SetUniformVariable("uMode", mode);
	OsuSphere(5, 50, 50);
	Pattern->Use(0);
	...
	glLoadIdentity();
	glColor3f(1., 1., 1.);
	DoRasterString(5., 5., 0., (char*)"Shaders Test");
...
InitMenus()
{
	...
	glutAddMenuEntry("No Animated Shaders", 0);
	glutAddMenuEntry("Animate Only Vertex Shader", 1);
	glutAddMenuEntry("Animate Only Fragment Shader", 2);
	glutAddMenuEntry("Aniamte Both Vertex & Fragment Shader", 3);
	...
	int projmenu = glutCreateMenu(DoProjMenu);
